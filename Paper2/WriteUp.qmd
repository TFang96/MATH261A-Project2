...
A limitation we still have is that while the log transformation reduced the heteroskedasticity in a substantial amount, some heteroskedasticity remains, particularly at the more extreme values. This would mean that the hypothesis tests using the p-values might still be slightly off.
